...
const ShowBlogs = () => {
  ...
  const fetchSingleBlog = async () => {
    try {
      await axios
        .get(`http://localhost:8000/api/getUsersBlog/${id}`)
        .then((res) => {
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-around">
        <div>
          <button
            className="px-4 inline py-2 text-sm font-medium leading-5 shadow text-white transition-colors duration-150 border border-transparent rounded-lg focus:outline-none focus:shadow-outline-blue bg-slate-600 active:bg-blue-600 hover:bg-blue-700"
          onClick={back}>Back</button>
        </div>
        <h2 className="  text-4xl p-5 text-center font-sans">
          My Blogs{" "}
        </h2>
      </div>
      {/* <ShowCategories/> */}
      <div className="flex bg-slate-700 w-full h-auto justify-around items-center flex-wrap">
        {blogData.length > 0 ? (
          blogData.map((elements) => {
            const { title, image, description, category } = elements;
                    ...
                    <div>
                      <div class="px-7  mb-8">
                        <div className="flex items-center justify-between h-10">
                          <div class=" px-2 uppercase cursor-pointer bg-red-700 max-w-min   rounded-lg text-gray-300 hover:bg-slate-800 hover:text-gray-100 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600 dark:hover:text-gray-200">
                            {category}
                          </div>
                        ...
                        <h2 class="text-3xl font-normal text-green-800 dark:text-gray-300">
                          {title}
                        </h2>
                        <p class="mt-2 text-gray-600 font-light dark:text-gray-300">
                          {description}
                          Lorem ipsum dolor sit amet consectetur adipisicing
                          elit. Corrupti, excepturi recusandae in fugiat
                          eligendi amet
                        </p>
                      </div>
                      <div className="flex items-center justify-end">
                        ...
                        <Link
                          // href="#"
                          to={`/blogs/${id}`}
                          // onClick={() => readMore(elements.id)}
                          class="group mb-4 mr-6 inline-flex items-center gap-1 text-sm font-medium text-blue-600"
                        >
                          Find out more
                          <span
                            aria-hidden="true"
            ...
            <div className="container flex flex-col md:flex-row items-center justify-center px-5 text-gray-700">
              <div className="max-w-md">
                ...
                <p className="mb-8 mt-4">
                  Visit Prifile section for creating your blog. or go to home to
                  explore blogs...
                </p>
                <Link
                  to="/"
